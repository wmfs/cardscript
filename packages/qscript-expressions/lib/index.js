...
class QScriptExpressionsxpressions {
  constructor () {
    this.asts = {}
  }

  getExpressionsFromQScript (qscript) {
    return qscript.widgets
  ...
  addQScript (qscript) {
    const expressions = this.getExpressionsFromQScript(qscript)
    this.convertExpressionsToAst(expressions)
  }

  calculateWidgetVisibility (qscript, data, asts) {
    let setVisible = [true]
    const visibilityList = {}
    let outerVis = true

    qscript.widgets.forEach(widget => {
...
module.exports = QScriptExpressionsxpressions
